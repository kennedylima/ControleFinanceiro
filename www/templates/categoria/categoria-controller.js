controleFinanceiroAPP.controller("CategoriaController", function ($scope,$rootScope,$state,$ionicPopup,$mdDialog,$cordovaSQLite,$cordovaToast,CategoriaFactory,CategoriaService) {
          ...
          .then(function(res) {
            if($scope.categorias.length != res.rows.length){
              for (var i=0; i < res.rows.length; i++){
                ...
                console.log("Index Categorias:" + i);
              }
            }
          }, function (error) {
            console.log(error);
    ...
    if(ionic.Platform.isAndroid()) {
      if($scope.categoria != undefined) {
        ...
        $cordovaSQLite.execute($rootScope.banco, query, [$scope.categoria.descricao, $scope.categoria.valorMaximo])
          .then(function (res) {
            $cordovaToast.showShortBottom("Salvo com sucesso" + res.insertId);
            CategoriaService.setCategoria(undefined);
            $state.go('categorias');
          }, function (err) {
            $cordovaToast.showShortBottom(err);
